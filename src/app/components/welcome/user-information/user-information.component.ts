...
@Component({
  ...
  template: `
    ...
    <div class="container text-center after-header">
      ...
      <div class="d-flex justify-content-center">
        <div class="card" *ngIf="user" style="width: 18rem;">
          ...
          <div class="card-body">
            @for (key of getObjectKeys(user); track $index) { @if (key !==
            'type_user') {
            <p class="card-text">
              <span class="fw-bold">{{ key }}:</span> {{ user[key] }}
            </p>

            } @else { @for (key2 of getObjectKeys(user[key]); track $index) {
            <p class="card-text">
              <span class="fw-bold">{{ key2 }}:</span> {{ user[key][key2] }}
            </p>
            } } }
